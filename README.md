...
- [Windows 다운로드](https://github.com/haju0423/poker-foundation/releases/download/2026.09.26-card-accusation/OneMoreCard-Windows.zip)
- [Mac 다운로드](https://github.com/haju0423/poker-foundation/releases/download/2026.09.26-card-accusation/OneMoreCard-Mac.zip)
...
1. 베팅 중 **NPC 멘트 넣기**를 누르거나 내 멘트를 보냅니다. NPC 멘트도 테이블과 이번 판 기록에 공개됩니다.
2. 체크·콜로 베팅을 마치면 원인이 될 멘트를 고르고 **변경 성공 / 요청 실패 / 시간 초과 재현**을 선택합니다. 멘트가 없으면 변경 성공은 선택할 수 없습니다.
3. 내 멘트로 변경했을 때만 카드에 본인 표시가 붙습니다. NPC 멘트로 변경한 카드는 내 화면에 표시되지 않습니다.
4. 공용 카드가 나오면 상대를 골라 **고발**하거나 **고발 안 함**을 누릅니다. NPC는 고발하지 않으며, 전원 응답 후 자동 마감합니다.
5. 고발했으면 **기록으로 판정**을 눌러 적중·오적중을 확인합니다. 실제 카드 변경이 없거나 다른 NPC를 지목하면 오적중입니다. 벌금·폴드·팟 지급 없이 결과 확인에서 멈추며 **새 게임**으로 다른 경우를 검사합니다.

고발하지 않았다면 **계속**으로 다음 베팅에 들어갑니다. 실제 AI나 싱글 캠페인이 아니라, 사람이 모이지 않아도 같은 포커·고발 로직을 확인하는 개발 화면입니다.
